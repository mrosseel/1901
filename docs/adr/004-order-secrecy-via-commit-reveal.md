...
- **r53, 2026-08-31** — The key is derived from the seat seed (ADR-049) and the phase, so any device holding that seed makes it again. The game master's own seat holds a token and no seed, so it makes a random key beside the draft.
- **r53, 2026-08-31** — The recovery needs no screen of its own. The seat's portable copy is already in the power menu with its QR code, and a seat-minted handover now carries the seed too (ADR-041), so a power handed over mid-phase keeps the orders it locked in. Only the phone can append a seed: the server holds the public half and nothing else, and a server that could write one into a link could read every envelope.
- **r53, 2026-08-31** — The menu needed a moment as well, because nobody opens a menu they have no question about. The seat page asks once, at the start of the game, with the address drawn so it can be bookmarked and a button that copies it, and never asks again. Bookmarking the board instead does not work: the key is taken out of the address as the board loads.
- **r53, 2026-08-31** — One case stays unrecoverable, and it is the price of the property: a dead phone whose seed was never copied. A game master mints a handover, the player is back in the game, and the orders that phone had locked in are beyond anybody's reach.
