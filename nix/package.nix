...
let
  ...
  webDeps = fetchNpmDeps {
    src = webSrc;
    hash = "sha256-VW7XiV779SoL6ID9O5vHDQJVEj0OxQ41toXglzPctrE=";
  };
  ...
  # Go sources only. Of web/ the Go build needs the two embed halves beside
  # the frontend and nothing else; tools/ is built alongside because it lives
  # in the same module.
  goSrc = lib.cleanSourceWith {
    ...
    filter =
      ...
      let
        name = baseNameOf path;
        underWeb = baseNameOf (dirOf path) == "web";
      in
      (!underWeb || (type == "regular" && lib.hasSuffix ".go" name))
      && !lib.elem name [
        "node_modules"
        ".github"
        ".claude"
