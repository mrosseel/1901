...
the spectator feed publishes them (ADR-013) — so this is arithmetic on what the
...
many supply centres do I have, and how many does the player across the table
have. Counting them off the map is what people do instead, and it is slow and
wrong about a supply centre that changed hands in the phase just gone.
*/
...
export interface Standing {
  ...
  supplyCentres: number;
  /** Units on the board. Fewer than supply centres means builds are owed. */
  units: number;
}

/*
Every power in the game, sorted by supply centres and then by name.
...
list of powers comes from the ownership map rather than from the units, because
a power can hold supply centres with no units standing and the reverse.
*/
export function standings(state: BoardState | null | undefined, powers?: string[]): Standing[] {
  // supplyCenters, American, is godip's word and the wire format's; the
  // prose either side of it is British (CONTEXT.md).
  const owned = state?.supplyCenters || {};
  const units = state?.units || {};

  const named = new Set<string>(powers || []);
  Object.values(owned).forEach((power) => named.add(power));
  Object.values(units).forEach((unit) => named.add(unit.nation));
  named.delete("");

  const rows = Array.from(named).map((power) => ({
    power: power,
    supplyCentres: Object.values(owned).filter((owner) => owner === power).length,
    units: Object.values(units).filter((unit) => unit.nation === power).length,
  }));

  return rows.sort((a, b) => {
    if (a.supplyCentres !== b.supplyCentres) return b.supplyCentres - a.supplyCentres;
...
export function buildBalance(row: Standing): number {
  return row.supplyCentres - row.units;
}
